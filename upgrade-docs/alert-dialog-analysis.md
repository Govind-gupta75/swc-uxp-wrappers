...
The `npm pack` diff between `@spectrum-web-components/alert-dialog@1.12.1` and `@1.12.2` showed
...
`packages/alert-dialog/package.json`'s `@swc-uxp-internal/alert-dialog` dependency value updated
from `1.12.1` to `1.12.2`. Wrapper `version` left unchanged. No wrapper source, CSS, or demo
files touched.
