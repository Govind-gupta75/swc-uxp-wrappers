# action-bar — 1.12.1 -> 1.12.2 (2026-08-27)

The `npm pack` diff between `@spectrum-web-components/action-bar@1.12.1` and `@1.12.2` showed only
...
pin bump only: `packages/action-bar/package.json`'s `@swc-uxp-internal/action-bar` dependency value was
updated from `npm:@spectrum-web-components/action-bar@1.12.1` to `@1.12.2`. The wrapper's own
